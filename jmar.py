...
def submit():
    ...
    c.execute("INSERT INTO Student=info VALUES(:f_name,:l_name,:age,:address,:email)",
            {
              'f_name':f_name.get(),
              'l_name':l_name.get(),
              'age':age.get(),
...
def query():
    conn=sqlite3.connect('C:/Users/STUDENTS/Desktop/personal_info.db')
    c=conn.cursor()
...
def delete ():
    conn=sqlite3.connect('C:/Users/STUDENTS/Desktop/personal_info.db')
    c=conn.cursor()
    c.execute("DELETE from Student=info WHERE oid="+delete_box.get())
...
def edit():
    ...
    record_id=delete_box.get()
    c.execute("SELECT * FROM Student=info WHERE oid=" +record_id)
    records=c.fetchall()
...
'''
c.execute("""CREATE TABLE "Student=info" (
	"f_name"	TEXT,
	"l_name"	TEXT,
	"age"	INTEGER,
	"address"	TEXT,
